...
def fetch_data(page):
    ...
    cookies = {
        "HMACCOUNT": "2336DB6C8C76EB56",
        "no-alert3": "true",
        "tk": "3750814722727243412",
        "Hm_lvt_c99546cf032aaa5a679230de9a95c7db": "1729752749,1731664155",
        "Hm_lvt_434c501fe98c1a8ec74b813751d4e3e3": "1731664172",
        "HMACCOUNT": "2336DB6C8C76EB56",
        "Hm_lpvt_434c501fe98c1a8ec74b813751d4e3e3": "1731664178",
        "m": "f9a74db56fcbd68038a8ecfc61f0e376|1731664215000",
        "Hm_lvt_9bcbda9cbf86757998a2339a0437208e": "1729752749,1731664234",
        "sessionid": "d0ztqnf09ei230ynuvifzuuw4p4at2ci",
        "Hm_lpvt_9bcbda9cbf86757998a2339a0437208e": "1732006483",
        "Hm_lpvt_c99546cf032aaa5a679230de9a95c7db": "1732006486",
    }
    params = {
        'page': page,
    }
...
def get_j_key(key, value):
    ...
    return js_code.playload_j_key(key, value)

def get_img_num_list(j_key, info):
    td_list = re.findall(r'<td>.*?</td>', info)
    sorted_list = []
    trun_num_list = []
    for td in td_list:
        img_list = re.findall(r'src="(data:.*?)" class="img_number (.*?)" style="left:(.*?)"', td)
        num_group = []
        index = 0
        for img in img_list:
            # 过滤
            if j_key not in img[1]:
                num_group.append({
                    # 获取 base64 对应的数字
                    'num': number_dict.get(img[0].replace('data:image/png;base64,', ''), ''),
                    # 用于排序的 left 值
                    'sort_left_val': float(img[2].replace('px', '')) + 11.5 * index,
                })
                index += 1
        # 排序
        sorted_list.append(sorted(num_group, key=lambda x: x['sort_left_val']))
    for group in sorted_list:
        trun_num_list.append(int(''.join([str(item['num']) for item in group])))
    return trun_num_list
...
def run_app(pages):
    ...
    for pid in range(1, pages + 1):
        ...
        if res.status_code == 200:
            j_key = get_j_key(res.json()['key'], res.json()['value'])
            res_info = res.json()['info']
            num_list = get_img_num_list(j_key, res_info)
            value += sum(num_list)
        else:
            print(f'{pid}页数据请求失败：{res.json()}')
    print(f'所有值的加和：{value}')
